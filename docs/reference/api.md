...
**IAA response** — pairwise matrix per annotator pair. For disentanglement: per-message accuracy after optimal thread alignment (o2o agreement). For adjacency pairs: LinkF1, TypeAcc, and Combined IAA = α × LinkF1 + (1 − α) × TypeAcc, where α is the project-level weight saved via `PUT /api/v1/admin/projects/{project_id}`.
